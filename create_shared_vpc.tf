...
resource "google_compute_subnetwork_iam_member" "cloudservices" {
  count      = "${length(var.service_projects)}"
  subnetwork = "${lookup(var.service_networks, element(keys(var.service_networks), count.index))}"
  role       = "roles/compute.networkUser"
  member     = "serviceAccount:${lookup(var.service_projects, element(keys(var.service_projects), count.index))}@cloudservices.gserviceaccount.com"
}
...
resource "google_compute_subnetwork_iam_member" "container-engine-robot" {
  ...
  subnetwork = "${lookup(var.service_networks, element(keys(var.service_networks), count.index))}"
  role       = "roles/compute.networkUser"
  member     = "serviceAccount:service-${lookup(var.service_projects, element(keys(var.service_projects), count.index))}@container-engine-robot.iam.gserviceaccount.com"
}
